 ...
 *                       ________
 *                      |        |
 * PWM_clock_src_xy -+->| /div_k |---> PWM_clock_x
 *                   |  |________|
 *                   |    ______
 *                   |   |      |
 ...
 *                       ________
 *                      |        |
 * PWM_clock_src_xy -+->| /div_k |---> PWM_clock_y
 *                   |  |________|
 *                   |    ______
 *                   |   |      |
				...
				bool en_bypass)
{
	...
	val = h616_pwm_readl(h616chip, PWM_XY_CLK_CR(PWM_PAIR(idx)));
	if (en_bypass)
		val |= BIT(PWM_XY_CLK_CR_BYPASS_BIT(idx));
	else
		val &= ~BIT(PWM_XY_CLK_CR_BYPASS_BIT(idx));

	h616_pwm_writel(h616chip, val, PWM_XY_CLK_CR(PWM_PAIR(idx)));
			      ...
			      struct pwm_state *state)
{
	...
	val = h616_pwm_readl(h616chip, PWM_XY_CLK_CR(PWM_PAIR(pwm->hwpwm)));
