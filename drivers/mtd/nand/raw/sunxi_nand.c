	...
	*cur_off = oob_off + ecc->bytes + user_data_sz;
	...
	if (ret < 0) {
		...
		if (oob_required) {
			...
			sunxi_nfc_hw_ecc_get_prot_oob_bytes(nand, oob, nfc_step, bbm,
							    page, user_data_sz);
		}
	}
...
/*
 * This returns the offset of the OOB for each step.
...
/*
 * This returns the offset of the ECC for each step.
