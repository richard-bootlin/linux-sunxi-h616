...
struct sunxi_nfc_caps {
	...
	int sram_size;
	unsigned int (*user_data_len)(int);
};
				       ...
				       bool bbm, bool oob_required, int page)
{
	...
	// TODO step was force 0 here
	sunxi_nfc_set_user_data_len(nfc, user_data_sz, step);
	...
	if (ret < 0) {
		...
		if (oob_required) {
			nand_change_read_column_op(nand, oob_off, NULL, 0,
						   false);
			sunxi_nfc_randomizer_read_buf(nand, oob,
						      ecc->bytes + user_data_sz,
						      true, page);
					    ...
					    int nchunks)
{
	...
	sunxi_nfc_reset_user_data_len(nfc);
	sunxi_nfc_set_user_data_len(nfc, nfc->caps->user_data_len(0), 0);
	sunxi_nfc_randomizer_config(nand, page, false);
	sunxi_nfc_randomizer_enable(nand);
					...
					int page)
{
	...
	sunxi_nfc_randomizer_enable(nand);
	sunxi_nfc_reset_user_data_len(nfc);
	// TODO: Why this step was forced 0 ?
	sunxi_nfc_set_user_data_len(nfc, user_data_sz, step);
	// TODO: Why this step was forced 0 ?
				    ...
				    struct mtd_oob_region *oobregion)
{
	...
	// TODO: I think the offset should be offset + user data
	// because just offset is the BBM, and the user data bytes are
	// at the beginning of the section.
	oobregion->offset = sunxi_get_ecc_offset(nfc, ecc, section) + user_data_sz;
				     ...
				     struct mtd_oob_region *oobregion)
{
	...
	/* The user data area is just before the ecc bytes */
